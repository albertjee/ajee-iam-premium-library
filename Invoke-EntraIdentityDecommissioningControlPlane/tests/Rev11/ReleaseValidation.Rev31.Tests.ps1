...
Describe 'Safety — Rev3.1 Write Isolation and Guest Safety Invariants' {
    ...
    BeforeAll {
        ...
        $script:EntryPointPath = Join-Path $PSScriptRoot `
    ...
    It 'Entry point ToolVersion is Rev3.1' {
        $content = Get-Content $script:EntryPointPath -Raw
        $content | Should -Match "\`$script:ToolVersion\s*=\s*'Rev3\.1'"
    }
...
Describe 'ReleaseValidation.psm1 — Rev3.1 Validation Checks' {
    ...
    It 'ReleaseValidation checks that ToolVersion is Rev3.1' {
        ...
        ($result.Errors | Where-Object { $_ -match 'Rev3\.1' }) | Should -Not -BeNullOrEmpty
    }
