...
Write-Host "Rev3.8 Push Readiness Validation" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Rev3.8 new files / directories to scan
$rev38Scope = @(
    (Join-Path $PSScriptRoot '..\src\Modules\NhiCredential.psm1'),
    (Join-Path $PSScriptRoot '..\src\Modules\NhiPermission.psm1'),
    (Join-Path $PSScriptRoot '..\src\Modules\NhiSignIn.psm1'),
    (Join-Path $PSScriptRoot '..\tests\NhiCredential.Rev38.Tests.ps1'),
    (Join-Path $PSScriptRoot '..\tests\NhiPermission.Rev38.Tests.ps1'),
    (Join-Path $PSScriptRoot '..\tests\NhiSignIn.Rev38.Tests.ps1')
)
...
Write-Host "STEP 2: Unicode/Mojibake Scan (Rev3.8 scope)" -ForegroundColor Green
Write-Host "---" -ForegroundColor Green
$unicodeViolations = @()
...
$rev38Files = $rev38Scope | ForEach-Object { Get-Item $_ -ErrorAction SilentlyContinue } | Where-Object { $_ -ne $null }

foreach ($file in $rev38Files) {
...
Write-Host "STEP 3: CRLF Line Ending Validation (Rev3.8 scope)" -ForegroundColor Green
Write-Host "---" -ForegroundColor Green
$crlfViolations = @()
...
if ($crlfViolations.Count -eq 0) {
    Write-Host "PASS: All Rev3.8 scoped files use CRLF line endings" -ForegroundColor Green
} else {
...
Write-Host "STEP 4: PowerShell AST Parse Check (all *.ps1/*.psm1/*.psd1)" -ForegroundColor Green
Write-Host "---" -ForegroundColor Green
$parseFailures = @()

$allSourceFiles = @()
$allSourceFiles += Get-ChildItem -Path (Join-Path $PSScriptRoot '..') -Recurse -Include *.ps1,*.psm1,*.psd1 -File -ErrorAction SilentlyContinue
...
if ($parseFailures.Count -eq 0) {
    Write-Host "PASS: All $($allSourceFiles.Count) source files parse cleanly" -ForegroundColor Green
} else {
    Write-Host "FAIL: Parse errors detected in $($parseFailures.Count) file(s)" -ForegroundColor Red
    $parseFailures | Format-Table -AutoSize
    $exitCode = 1
}
Write-Host ""

Write-Host "STEP 5: Module Import Smoke Test (Rev3.8 NHI modules)" -ForegroundColor Green
Write-Host "---" -ForegroundColor Green
$importFailures = @()

foreach ($modulePath in $newModuleFiles) {
...
if ($importFailures.Count -eq 0) {
    Write-Host "PASS: All Rev3.8 modules import successfully" -ForegroundColor Green
} else {
...
if ($RunPester) {
    ...
    $result = Invoke-Pester -Path (Join-Path $PSScriptRoot '..\tests') -PassThru -ErrorAction Continue
    ...
    Write-Host "Command: .\tools\Test-Rev38PushReadiness.ps1 -RunPester" -ForegroundColor Cyan
}
Write-Host ""
