...
function Export-DecomTraceabilityReportMarkdown {
    <#
    .SYNOPSIS
    Exports a traceability model to a Markdown file.
    .PARAMETER Model
    The traceability model returned by New-DecomTraceabilityModel.
    .PARAMETER Path
    ...
    $lines.Add('# Traceability Report')
    $lines.Add('')
    $lines.Add('| Field | Value |')
    $lines.Add('|---|---|')
    $lines.Add("| SchemaVersion | $($Model.SchemaVersion) |")
    $lines.Add("| ToolVersion | $($Model.ToolVersion) |")
    $lines.Add("| RunId | $($Model.RunId) |")
    $lines.Add("| GeneratedUtc | $($Model.GeneratedUtc) |")
    $lines.Add('')
    $lines.Add('## Summary')
    $lines.Add('')
    $lines.Add('| Category | Count |')
    $lines.Add('|---|---|')
    $lines.Add("| Total | $($Model.Summary.Total) |")
    $lines.Add("| FindingOnly | $($Model.Summary.FindingOnly) |")
    $lines.Add("| WhatIfGenerated | $($Model.Summary.WhatIfGenerated) |")
    $lines.Add("| Approved | $($Model.Summary.Approved) |")
    $lines.Add("| Executed | $($Model.Summary.Executed) |")
    $lines.Add("| Skipped | $($Model.Summary.Skipped) |")
    $lines.Add("| Blocked | $($Model.Summary.Blocked) |")
    $lines.Add("| Failed | $($Model.Summary.Failed) |")
    $lines.Add("| EvidenceMissing | $($Model.Summary.EvidenceMissing) |")
    $lines.Add("| TraceGap | $($Model.Summary.TraceGap) |")
    $lines.Add('')
    $lines.Add('## Trace Rows')
    $lines.Add('')
    $lines.Add('| FindingId | Severity | DisplayName | ActionId | ActionType | ApprovalStatus | ExecutionOutcome | TraceStatus | TraceGapReason |')
    $lines.Add('|---|---|---|---|---|---|---|---|---|')

    foreach ($row in $Model.Rows) {
        $fid   = [string]$row.FindingId
        $sev   = [string]$row.Severity
        $obj   = ([string]$row.DisplayName) -replace '\|', '&#124;'
        $aid   = [string]$row.ActionId
        $atype = [string]$row.ActionType
        $appr  = [string]$row.ApprovalStatus
        $exec  = [string]$row.ExecutionOutcome
        $ts    = [string]$row.TraceStatus
        $tgr   = ([string]$row.TraceGapReason) -replace '\|', '&#124;'
        $lines.Add("| $fid | $sev | $obj | $aid | $atype | $appr | $exec | $ts | $tgr |")
    }
