...
function New-DecomRev3_3WriteReadinessReport {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [PSObject]$Context
    ...
    $candidates = @(
        [PSCustomObject]@{
            CandidateActionType = 'AddApplicationOwner'
            FindingIds = @('DEC-APP-001','DEC-APP-002','DEC-APP-003','DEC-SPN-001')
            Status = 'Candidate'
            Recommendation = 'ReadyForRev3.3Design'
            Notes = 'Requires exact NewOwnerObjectId from approval manifest. No inferred owner.'
            RiskLevel = 'Medium'
            RequiresDesignBefore = 'Rev3.3'
        },
        [PSCustomObject]@{
            CandidateActionType = 'RemoveCAExclusionGroupMember'
            FindingIds = @('DEC-CA-001','DEC-CA-002','DEC-CA-003','DEC-CA-004')
            Status = 'Candidate'
            Recommendation = 'ReadyForRev3.3Design'
            Notes = 'No CA policy mutation in Rev3.2. Read-only governance only.'
            RiskLevel = 'High'
            RequiresDesignBefore = 'Rev3.3'
        },
        [PSCustomObject]@{
            CandidateActionType = 'RemoveNonExpiredCredentialAfterRotationEvidence'
            FindingIds = @('DEC-APP-004')
            Status = 'NeedsDesign'
            Recommendation = 'ReadyForRev3.3Design'
            Notes = 'Only applicable when rotation evidence is present. Rev3.2 does not implement.'
            RiskLevel = 'High'
            RequiresDesignBefore = 'Rev3.3'
        },
        [PSCustomObject]@{
            CandidateActionType = 'DisableApplication'
            FindingIds = @('DEC-APP-001')
            Status = 'Deferred'
            Recommendation = 'ReadyForRev3.3Design'
            Notes = 'Not recommended in Rev3.2. Requires separate QA and stakeholder approval.'
            RiskLevel = 'Critical'
            RequiresDesignBefore = 'Rev3.3'
        },
        [PSCustomObject]@{
            CandidateActionType = 'DisableServicePrincipal'
            FindingIds = @('DEC-SPN-001')
            Status = 'Deferred'
            Recommendation = 'ReadyForRev3.3Design'
            Notes = 'Not recommended in Rev3.2. Requires separate QA and stakeholder approval.'
            RiskLevel = 'Critical'
    ...
    $report = [PSCustomObject]@{
        ...
        Notes            = 'Rev3.2 delivers RemoveExpiredApplicationCredential. Rev3.3 candidates require separate design and QA.'
    }

    return $report
}

function Export-DecomRev3_3WriteReadinessJson {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [PSObject]$Report,
    ...
    $Timestamp = Get-Date -Format 'yyyyMMdd_HHmmss'
    $JsonPath  = Join-Path $Context.OutputPath "rev3.3-write-readiness-report-$Timestamp.json"
    $Report | ConvertTo-Json -Depth 10 | Out-File -FilePath $JsonPath -Encoding UTF8
    Write-DecomOk "Rev3.3 write-readiness JSON: $JsonPath"
}

function Export-DecomRev3_3WriteReadinessMarkdown {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [PSObject]$Report,
    ...
    $Timestamp = Get-Date -Format 'yyyyMMdd_HHmmss'
    $MdPath    = Join-Path $Context.OutputPath "rev3.3-write-readiness-report-$Timestamp.md"

    $candidateRows = ($Report.CandidateActions | ForEach-Object {
        "| $($_.CandidateActionType) | $($_.Status) | $($_.RiskLevel) | $($_.Recommendation) |"
    }) -join "`n"
...
| Action Type | Status | Risk Level | Recommendation |
|---|---|---|---|
$candidateRows
...
"@
    ...
    $markdown | Out-File -FilePath $MdPath -Encoding UTF8
    Write-DecomOk "Rev3.3 write-readiness Markdown: $MdPath"
}
